...
// تعريف أنواع المستخدمين وصلاحياتهم
...
const rolePermissions: Record<ManagerRole, Permission[]> = {
  super_manager: [
    "read_dashboard", "manage_products", "manage_orders", "manage_customers",
    "manage_staff", "view_analytics", "manage_finances", "manage_marketing",
    "manage_content", "system_settings", "user_management", "export_data", "manage_inventory"
  ],
  store_manager: [
    "read_dashboard", "manage_products", "manage_orders", "manage_customers",
    "view_analytics", "manage_inventory", "export_data"
  ],
  operations_manager: [
    "read_dashboard", "manage_orders", "manage_customers", "view_analytics", 
    "manage_inventory", "export_data"
  ],
  marketing_manager: [
    "read_dashboard", "view_analytics", "manage_marketing", "manage_content", 
    "manage_customers", "export_data"
  ],
  finance_manager: [
    "read_dashboard", "view_analytics", "manage_finances", "export_data"
  ],
  content_manager: [
    "read_dashboard", "manage_content", "manage_products", "view_analytics"
  ]
};

// بيانات وهمية للمديرين
const mockManagers: (Omit<ManagerUser, 'permissions'> & { password: string })[] = [
  {
    id: "1",
...
// إنشاء متجر المصادقة مع التخزين المحلي
...
export const ProtectedRoute = ({ 
  ...
  const { isAuthenticated, hasPermission } = useManagerAuth();
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            تحتاج إلى صلاحية: {getPermissionDisplayName(requiredPermission)}
          </p>
        </div>
      </div>
    );
